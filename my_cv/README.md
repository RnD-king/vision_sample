...
> 이렇게 세 폴더를 싹 지우고 빌드하는 걸 클린빌드라고 부르며,  
> 빌드 오류나 실행파일이 좀 이상하다 싶을 때 클린빌드로 해결하는 경우가 많다)
...
단순한 기능이니 추가로 원하는 것이 있다면 활용하여 본인만의 코드를 만들어보자.  
...
다만 코드에 값을 직접 넣는 방법은 색을 바꿀 때마다 재빌드가 필요하므로 rqt를 애용하자.  

> 파라미터 rqt 사용법  
> 새 터미널  >  source~  >  rqt입력 > 상단 메뉴의 Plugins-Configuration-Dynamic Reconfigure 클릭  
> 왼쪽 토픽에서 color_test_node를 클릭한 후, 파라미터 값을 바꿔보자  
> 간혹 알 수 없는 이유로 Dynamic Reconfigure를 클릭해도 아무것도 뜨지 않을 때가 있는데, 그냥 뜰 때까지 계속 눌러주자

출력 화면을 클릭한 부분의 HSV값을 터미널로 띄워주니 색상표 대신 그걸 보고 해도 좋다.  
다른 창에선 깊이 범위도 확인할 수 있다.  
...
\* 듀얼 카메라를 사용한다면 ball_and_hoop의 카메라 나누는 구성을 참고해봐도 좋지만, 분명! 더 좋은 방법이 있다
...
조악하지만 가볍고 단순하게 구현하기엔 이게 제일 낫다고 생각했다,,
...
유효한 값 범위 내에서만 적용되게 만들고 싶다면 지금처럼 if, else문의 반복 필요하지만,
```python
def parameter_callback(self, params):
   for param in params:
...
답:  
맞는 말이다. 굳이 line_subscriber에도 이런 로직이 있을 필요는 없다.  

언젠가 넣어놨다가 나중에 귀찮아서 빼지 않았다  
굳이굳이 또 이유를 찾자면,
정말 만에 하나 LinePointsArray 메세지가 타이밍이 꼬여서 잘못 들어오는 불상사가 발생하더라도 이를 막을 안전장치가 될 수 있기 때문에 남겨두었다.

참고로 현재 line_publisher는 축약된 버전이라 이 로직이 없지만, 실제라면 구현되어있어야 한다.  
